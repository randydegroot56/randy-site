...
const agents = [
  ...
  },
  {
    ...
    phases: [
      ...
      { label: 'Phase 2 — Detection', status: '✅ Live', detail: 'Cross-references the dependency map against entry points to find code imported nowhere — unused exports, orphan files, dead variables.' },
      { label: 'Phase 3 — Verification', status: '✅ Live', detail: 'Safety check before any removal: re-examines flagged items with additional heuristics to eliminate false positives. Six-layer pre-flight check.' },
      { label: 'Phase 4 — Reporting', status: '✅ Live', detail: 'Generates HTML dashboard, CSV export, and JSON summary with per-file recommendations and risk assessments.' },
      { label: 'Phase 5 — Execution', status: '✅ Live', detail: 'Staged, reversible removal of confirmed dead code. Every change is committed atomically with a rollback path.' },
    ],
    commands: [
      { label: 'Discover (Phase 1)', cmd: 'python -m agents.code_auditor.cli discover --project . --output audit_report.json' },
      { label: 'Detect unused (Phase 2)', cmd: 'python -m agents.code_auditor.cli detect --report audit_report.json --output phase2_findings.json' },
      { label: 'Verify safety (Phase 3)', cmd: 'python -m agents.code_auditor.cli verify --report phase2_findings.json --output phase3_verified.json' },
      { label: 'Execute removals (Phase 5)', cmd: 'python -m agents.code_auditor.cli execute --report phase3_verified.json --dry-run' },
    ],
  },
  {
    id: 'code-fixer',
    ...
    phases: [
      { label: 'Analyze', status: '✅ Live', detail: 'Reads phase3_verified.json and categorizes findings by type and risk. Shows a summary — LOW/MEDIUM item counts, estimated lines, batch count — before any changes are made.' },
      { label: 'Plan', status: '✅ Live', detail: 'Groups findings into batches of configurable size, ordering by risk level (low → high). Writes a JSON plan file showing exactly what will change, with no files touched.' },
      { label: 'Fix', status: '✅ Live', detail: 'Invokes the Code Auditor\'s Phase 5 executor per batch. Tests run after each batch. On failure, the batch is rolled back automatically via git.' },
      { label: 'Verify', status: '✅ Live', detail: 'Six-layer pre-flight check per item: report integrity, clean git tree, ID existence, risk threshold, dry-run impact, baseline test pass.' },
    ],
    commands: [
      { label: 'Analyze findings', cmd: 'python agents/code_fixer/cli.py analyze --report phase3_verified.json' },
      { label: 'Plan batches (dry run)', cmd: 'python agents/code_fixer/cli.py plan --report phase3_verified.json' },
